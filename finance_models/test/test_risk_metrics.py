...
import numpy as np
import pandas as pd
from .. import risk_metrics, tools
import unittest
...
class TestVAR(unittest.TestCase):
    def setUp(self):
        ...
        self.var = risk_metrics.VaR(
            securities_values = self.secs_values,
            notionals = 1000
        )
    
    def test_calc_retorno_class(self):
        ret = self.var.calcula_retorno()

        ret_1 = ret.iloc[1]
        ret_1_expected = 0.025218 # calc Excel

        self.assertAlmostEqual(
            ret_1, ret_1_expected, places = 6,
            msg = f"Wrong first return. Got {ret_1:.2%}, expected {ret_1_expected:.2%}"
        )

        ret_final = (ret + 1).prod() - 1
        ret_final_expected = 24.43329  # calc Excel

        self.assertAlmostEqual(
            ret_final, ret_final_expected, places = 5,
            msg = f"Wrong total returns. Got {ret_final:.2%}, expected {ret_final_expected:.2%}"
        )
    
    def test_calc_retorno_func(self):
        precos = self.var.portfolio_total

        ret = risk_metrics.VaR().calcula_retorno(precos = precos, holding_period = 1)
        ...
        self.assertAlmostEqual(
            ret_1, ret_1_expected, places = 6,
            msg = f"Wrong first return. Expected {ret_1:.2%}, got {ret_1_expected:.2%}"
        )
    ...
    def test_calc_logretorno_class(self):
        ret = self.var.calcula_log_retorno()

        ret_1 = ret.iloc[1]
        ret_1_expected = 0.024906 # calc Excel

        self.assertAlmostEqual(
            ret_1, ret_1_expected, places = 6,
            msg = f"Wrong first log return. Got {ret_1:.2%}, expected {ret_1_expected:.2%}"
        )

        ret_final = ret.sum()
        ret_final_expected = 3.23606 # calc Excel

        self.assertAlmostEqual(
            ret_final, ret_final_expected, places = 5,
            msg = f"Wrong total log returns. Got {ret_final:.2%}, expected {ret_final_expected:.2%}"
        )
    
    def test_calc_logretorno_func(self):
        precos = self.var.portfolio_total

        ret = risk_metrics.VaR().calcula_log_retorno(precos = precos, holding_period = 1)
    ...
    def test_calc_var_class(self):
        alpha = 0.04
        var_final = tools.Money(self.var.calcula_var(alpha = alpha))
        var_final_expected = tools.Money(-17509.53) # calc Excel

        self.assertAlmostEqual(
            var_final, var_final_expected, delta = tools.Money(0.01),
            msg = f"Wrong VaR {1-alpha:.1%} (full returns). Got {var_final}, expected {var_final_expected}"
        )
    
    def test_calc_var_func(self):
        alpha = 0.04

        ret = self.var.calcula_retorno()
        vlr_carteira = self.var.portfolio_total.iloc[-1]
        
        var_final = tools.Money(risk_metrics.VaR().calcula_var(
            retornos = ret.iloc[:-1],
            alpha = alpha,
            vlr_carteira_atual = vlr_carteira
        ))
    ...
    def test_calc_es_class(self):
        alpha = 0.04
        es_final = tools.Money(self.var.calcula_es(alpha = alpha))
        es_final_expected = tools.Money(-40162.66) # calc Excel

        self.assertAlmostEqual(
            es_final, es_final_expected, delta = tools.Money(0.01),
            msg = f"Wrong ES {1-alpha:.1%} (full returns). Got {es_final}, expected {es_final_expected}"
        )
    
    def test_calc_es_func(self):
        alpha = 0.04

        ret = self.var.calcula_retorno()
        vlr_carteira = self.var.portfolio_total.iloc[-1]
        
        es_final = tools.Money(risk_metrics.VaR().calcula_es(
            retornos = ret.iloc[:-1],
            alpha = alpha,
            vlr_carteira_atual = vlr_carteira
        ))
    ...
    def test_calc_pnl_func(self):
        ...
        pnl = risk_metrics.VaR().calcula_pnl(
            precos = precos,
            holding_period = 1
        )
